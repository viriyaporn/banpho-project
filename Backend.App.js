...
//         return res.json({ status: "error", message: "User not found" });
//       }
//       const isMatch = bcrypt.compare(password, results[0].user_password);//ถอดรหัสรหัสผ่านเลขมั่วๆในฐานข้อมูล
//       console.log(isMatch)
//       if (isMatch) { //เช็คว่ารหัสที่ถอดมาได้ตรงกับที่เรากรอกloginมาไหม
//         return res.json({ status: "ok", data: results });
//       } else {
      ...
      connection.query(
        ...
        function (err, results) {
          ...
          const title_notification = `รับอุปกรณ์-เครื่องมือการแพทย์สำเร็จ กรุณามารับคืน วันที่ ${moment(
            tracking_meet
          ).locale('th').add(543, 'year').format("DD-MM-YYYY")}`;
...
app.get("/document-card-manager/:id/:role/:status", jsonParser, (req, res) => {
  ...
  if (status != 0) {
    ...
    if(id != 17){
      connection.query(
        "SELECT COUNT(*) as COUNT FROM document WHERE document_status = ? AND hospital_id = ?",
        [newStatus,id],
        function (err, results) {
          if (err) {
            res.json({ status: "error", message: err });
      ...
      connection.query(
        "SELECT COUNT(*) as COUNT FROM document WHERE document_status = ?",
        [newStatus],
        function (err, results) {
          if (err) {
            res.json({ status: "error", message: err });
...
app.post("/document", jsonParser, (req, res) => {
  ...
  const code_view = req.body.code_view;
  ...
  connection.query(
    "INSERT INTO document (document_code,document_code_view,document_title,document_detail,document_file,document_file_path,document_version,document_status,created_at,hospital_id,created_by,user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
    [
      code,
      code_view,
      title,
...
app.get("/documents-disapprove/:id/:role", jsonParser, (req, res) => {
  ...
  if (hospital_id != 17) {
    connection.query(
      "SELECT * FROM approval INNER JOIN document ON document.document_code = approval.document_code INNER JOIN hospital ON hospital.hospital_id = approval.approval_hospital  WHERE approval.approver_id = ? AND approval.approval_status = ? AND approval_hospital = ?",
      [approval_status, 2, hospital_id],
      function (err, results) {
        if (err) {
          res.json({ status: "error", message: err });
          return;
    ...
    connection.query(
      "SELECT * FROM approval INNER JOIN document ON approval.document_code = document.document_code INNER JOIN hospital ON document.hospital_id = hospital.hospital_id  WHERE approval.approver_id = ? AND approval.approval_hospital = ? AND approval.approval_status = ?",
...
app.get("/download-file", (req, res) => {
  ...
  res.download(`../database-backend${file_path}`, file_name, (err) => {
    if (err) {
      console.log(err);
